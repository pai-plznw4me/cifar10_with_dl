...
for i in range(1, 6):
    ...
    xs = unpickle('/content/cifar-10-batches-py/data_batch_{}'.format(i))[b'data'].reshape((10000, 3 ,32 ,32)).transpose((0, 2, 3, 1))
    xs = xs.reshape([-1, 3*1024])
    xs_bucket.append(xs)
...
# test 데이터 셋 가져오기
test_xs = unpickle('/content/cifar-10-batches-py/test_batch')[b'data'].reshape((10000, 3 ,32 ,32)).transpose((0, 2, 3, 1))
test_xs = test_xs
...
plt.imshow(train_xs[2])

input_ = Input(shape=(32, 32, 3))

# block 1
layer = Conv2D(filters=16, kernel_size=3, strides=1, padding='same', 
               activation='relu', kernel_initializer='he_normal')(input_)
layer = MaxPool2D(strides=2)(layer)

# block 2
layer = Conv2D(filters=32, kernel_size=3, strides=1, padding='same', 
...
# block 3
layer = Conv2D(filters=64, kernel_size=3, strides=1, padding='same', 
               activation='relu', kernel_initializer='he_normal')(layer)
